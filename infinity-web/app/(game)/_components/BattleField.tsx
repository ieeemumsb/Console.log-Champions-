...
export function BattleField({
  ...
  function attack(): void {
    ...
    setTimeout(() => {
      if (Math.random() < 0.5) {
        ...
        setBattleLog((prev) => [
          ...prev,
          `You hit the ${monster} for ${player.power} damage!`,
        ]);
        ...
        setBattleLog((prev) => [
          ...prev,
          `You missed! The ${monster} counterattacks!`,
        ]);
      }
      setIsAnimating(false);
    }, 500);
  ...
  function monsterDefeated(): void {
    ...
    if (newPlayer.level > oldLevel) {
      setLevelUpMessage(
        `🎉 Level Up! You are now level ${newPlayer.level}! 🎉`
      );
      if (window.playSoundEffect) {
  ...
  return (
    <div className="space-y-6">
      ...
      {currentMonster &&
        ...
        !lostGame && (
          <div
            className={`battle-field rounded-2xl p-8 ${isAnimating ? "battle-animation" : ""}`}
          >
            ...
            <div className="flex flex-wrap justify-center gap-4">
              <button
                onClick={runAway}
                className="button-danger text-white font-bold py-3 px-6 rounded-xl battle-field-button"
              >
                🏃 Run Away
              </button>
              <button
                onClick={block}
                className="button-secondary text-white font-bold py-6 px-6 rounded-xl battle-field-button"
              >
                🛡️ Block
              </button>
              <button
                onClick={attack}
                className="button-primary text-white font-bold py-6 px-6 rounded-xl battle-field-button"
              >
                ⚔️ Attack
              </button>
            </div>
        ...
        !lostGame && (
          <div className="victory-field rounded-2xl p-8 text-center">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-yellow-400 font-semibold">
                      +{currentMonster.power} Gold
                    </p>
                  </div>
                  <div>
                    <div className="text-2xl text-blue-400 mb-1">⭐</div>
                    <p className="text-blue-400 font-semibold">
                      +{Math.floor(currentMonster.power * 1.5)} XP
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="flex flex-wrap justify-center gap-4">
              ...
              <button
                onClick={backToWorld}
                className="button-secondary text-white font-bold py-6 px-6 rounded-xl"
              >
                🌍 Return to World
              </button>
            </div>
