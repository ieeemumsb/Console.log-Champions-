...
import { generateAlerts, cn } from "@/lib/utils"
import { DepartmentSummary, AlertItem } from "@/lib/type"
...
export default function Dashboard() {
    ...
    const companyTotals = dashboardData.reduce((acc, dept) => ({
    ...
    const overallProfitMargin = companyTotals.totalRevenue > 0
            ...
            <div className="min-h-screen bg-background p-6">
                <div className="max-w-7xl mx-auto">
                    <div className="mb-8">
                        ...
                        <p className="text-xl text-muted-foreground mt-2">Financial Analytics Dashboard</p>
                    </div>
            ...
            <div className="max-w-7xl mx-auto space-y-6">
                ...
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-lg text-muted-foreground mt-1">Financial Analytics Dashboard</p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    <MetricCard
                        ...
                        value={alerts.length}
                        format="number"
                        trend={alerts.filter(a => a.type === 'critical').length > 0 ? 'down' : 'stable'}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {dashboardData.map((dept, index) => (
                                <MetricCard
                                    ...
                                    changeType="percentage"
                                    trend={dept.profitMargin > 15 ? 'up' : dept.profitMargin < 5 ? 'down' : 'stable'}
                                    icon={<Building2 className="h-5 w-5" />}
                                    className={cn(
                                        "transition-all duration-200",
                                        index === 0 && "ring-2 ring-blue-500/20 ring-offset-2 ring-offset-background"
                                    )}
                                />
                            ))}
                        </div>
                                    ...
                                    {['critical', 'warning', 'info'].map(type => {
                                        ...
                                        return (
                                            <div key={type} className="flex items-center justify-between p-3 rounded-lg border">
                                                <span className="capitalize font-medium">{type} Alerts</span>
                                                <Badge variant={type === 'critical' ? 'destructive' : 'secondary'}>
                                                    {count}
                                                </Badge>
                                            </div>
                                        )
