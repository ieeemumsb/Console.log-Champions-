...
export async function cleanFinancialData(rawData: any[]): Promise<FinancialRecord[]> {
    try {
...
- Return valid JSON array only - no explanations or comments`;
        ...
        try {
            ...
            const jsonStart = Math.max(responseContent.indexOf('['), responseContent.indexOf('{'));
            if (jsonStart > 0) {
            ...
            const cleanedData = JSON.parse(responseContent);
            return Array.isArray(cleanedData) ? cleanedData : [];
        } catch (parseError) {
            console.error('JSON parsing failed, falling back to manual cleaning:', parseError);
...
export async function generateForecast(params: { period: number; department: string }): Promise<any> {
    try {
        const prompt = `
