...
import { getMonsters, monsterHit, resetMonsters } from "../_lib/Monsters";
...
export function GameDisplay(){
    ...
    return (
        <div className="w-full max-w-4xl mx-auto relative">
            ...
            {/* Store Button */}
            {gameStarted && !battleStarted && !lostGame && !wonGame && (
                <button
                    onClick={() => setStoreOpen(true)}
                    className="fixed top-20 right-20 z-40 button-secondary text-white font-bold py-3 px-6 rounded-xl hover:scale-105 transition-all duration-200"
                >
                    🏪 Store
                </button>
            )}
            ...
            {!battleStarted && !gameStarted && !lostGame && !wonGame && (
                <div className="w-full bg-red-200 glass-effect rounded-2xl shadow-2xl p-8 max-w-md mx-auto text-center">
            ...
            {!battleStarted && gameStarted && !lostGame && !wonGame && (
                <div className="glass-effect rounded-2xl shadow-2xl p-8 max-w-4xl mx-auto">
                    <Map setBattleStarted={setBattleStarted} setMonster={setMonster} />
                </div>
            )}
            ...
            {lostGame && !battleStarted && !gameStarted && (
                <div className="glass-effect rounded-2xl shadow-2xl p-8 max-w-md mx-auto text-center">
                    <div className="mb-6">
            ...
            {wonGame && !battleStarted && !lostGame && (
                <div className="glass-effect rounded-2xl shadow-2xl p-8 max-w-md mx-auto text-center">
                    <div className="mb-6">
                        ...
                        <p className="text-gray-300 mb-6">All monsters have been defeated!</p>
                        <p className="text-xl text-green-400 mb-4">You are the ultimate Guardian!</p>
                        <p className="text-sm text-gray-400 mb-6">
                            Final Stats: {player.xp} XP • {player.gold} Gold • Level {player.level}
                        </p>
                    </div>
                    <button 
                        onClick={resetGame} 
